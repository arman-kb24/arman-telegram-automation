...
function tdjsonCompat(td) {
  var _getName, _ref;
  const foundTdlTdlibAddon = ((_getName = (_ref = td).getName) === null || _getName === void 0 ? void 0 : _getName.call(_ref)) === 'addon';
...
const TDLIB_DEFAULT = new _version.Version('1.8.19');
...
class Client {
  _emitter = new _eventemitter.default();
  _pending = new Map();
  _requestId = 0;
  _client = null;
  _initialized = false;
  ...
  _authNeeded = false;
  _loginDefer = new TdlDeferred();
  _version = TDLIB_DEFAULT;
  _tdn = false;
  _onClose = () => {};
  _clientId = -1;
  constructor(tdlibInstance, options = {}, managing) {
    ...
    if (!options.nodeVersion) this._options.nodeVersion = "NONE";
    debug("options.addonVersion=", options.addonVersion);
    if (!options.addonVersion) this._options.addonVersion = "NONE";
    debug("options.binaryVersion=", options.binaryVersion);
    if (!options.binaryVersion) this._options.binaryVersion = "NONE";
    ...
    if (!this._tdn) {
      this._client = this._tdlib.create(this._options.receiveTimeout, this._options.nodeVersion, this._options.addonVersion, this._options.binaryVersion);
  ...
  login = (getLoginDetails = emptyDetails) => {
    debug('client.login()');
  ...
  /** @deprecated */
  connectAndLogin = (fn = emptyDetails) => {
  ...
  execute = request => {
    ...
    if (this._tdn) {
      ...
      return (0, _util.deepRenameKey)('@type', '_', JSON.parse(tdResponse));
    }
    // the client can be null, it's fine
    const tdResponse = this._tdlib.execute(this._client, tdRequest);
    if (tdResponse == null) return null;
    return (0, _util.deepRenameKey)('@type', '_', tdResponse);
  };
  _send(request) {
    debugReq('send', request);
    const tdRequest = (0, _util.deepRenameKey)('_', '@type', request);
    if (this._client === null && this._clientId === -1) throw new Error('A closed client cannot be reused, create a new Client');
    if (this._tdn) this._tdlib.tdn.send(this._clientId, JSON.stringify(tdRequest));else this._tdlib.send(this._client, tdRequest);
  }
  _sendTdl(request) {
    this._send({
      ...request,
  ...
  async _loop() {
    while (true) {
      ...
      try {
        this._handleReceive((0, _util.deepRenameKey)('@type', '_', res));
      } catch (e) {
    ...
    switch (authorizationState._) {
      case 'authorizationStateWaitTdlibParameters':
        if (this._version.gte(TDLIB_1_8_6)) {
          if (this._options.tdlibParameters._ != null) throw new Error('tdlibParameters must not contain the _ property');
          this._sendTdl({
